...
export const fhevmMockCreateInstance = async (parameters: {
  ...
  // Use gatewayChainId from metadata if available, otherwise extract from EIP712 domain
  // EIP712 domain structure: [fields, name, version, chainId, verifyingContract, salt, extensions]
  const gatewayChainId = parameters.metadata.gatewayChainId ?? Number(domain[3] as bigint);
